...
const nav = [
  ...
  { href: "/dashboard/data-room", label: "Data room", icon: FolderLock },
  { href: "/dashboard/data", label: "Data pipeline", icon: Database },
    ...
    <div className="flex min-h-screen bg-surface-page">
      <aside className="hidden w-56 shrink-0 flex-col border-r border-border bg-surface-elevated lg:flex">
        <div className="border-b border-border px-5 py-4">
          <Logo href="/dashboard" />
          <p className="mt-3 truncate text-xs text-text-tertiary">{orgName}</p>
        </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 items-center justify-between border-b border-border bg-surface-elevated px-4 lg:px-8">
          ...
          <p className="hidden text-sm text-text-tertiary lg:block">Investor CRM & outreach</p>
          <Link
            href="/request-access"
